...
from dataloaders.WSI_Loader import tcga_lung_dataloader
...
class PatchEncoder(nn.Module):
    ...
    def forward(self, patches):
        preprocessed_patches = (torch.stack([self.preprocess(transforms.ToPILImage()(img)) for img in patches]).
                                to(self.device))
        patch_embeddings = self.conch_model.visual.forward_no_head_2(preprocessed_patches, normalize=False)
...
if __name__ == '__main__':
    ...
    """
    Repeat this process for Train and Validation folds and the test sets. The csv files are included in csv_files 
    folder. Using them create h5 folders and WSI folders.
    """

    # Add the path to Train Fold 1 h5 files
    h5_dir_fold_1_train = ''
    # Add the path to Train Fold 1 WSI files
    wsi_dir_fold_1_train = ''

    wsi_loader_fold_1_train = tcga_lung_dataloader(h5_dir=h5_dir_fold_1_train, wsi_dir=wsi_dir_fold_1_train)

    tensors_fold_1_train_save_dir = ''
    os.makedirs(tensors_fold_1_train_save_dir, exist_ok=True)

    for batch_idx, (regions, label, image_name) in enumerate(wsi_loader_fold_1_train):
        regions_embedding = embed_patches(regions, patch_encoder)
        tensor_per_image = regions_embedding.squeeze(0)
        tensors_save_path = os.path.join(tensors_fold_1_train_save_dir, image_name[0].replace('.png', '.pt'))
        data = {
