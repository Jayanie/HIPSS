...
"""
Text Encoder Model Modified for Scaling and Shifting Features (SSF)-based tuning
"""
...
class CustomMLP(nn.Module):
    def __init__(self, d_model, mlp_width, act_layer, tuning_mode=None, init_ssf=True):
        super().__init__()
        ...
        if tuning_mode == 'ssf':
            self.ssf_scale_1, self.ssf_shift_1 = init_ssf_scale_shift(mlp_width, init_ssf=init_ssf)
            self.ssf_scale_2, self.ssf_shift_2 = init_ssf_scale_shift(d_model, init_ssf=init_ssf)
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        if self.tuning_mode == 'ssf':
            self.ssf_scale_1, self.ssf_shift_1 = init_ssf_scale_shift(d_model, init_ssf=init_ssf)
            self.ssf_scale_2, self.ssf_shift_2 = init_ssf_scale_shift(d_model, init_ssf=init_ssf)
        self.mlp = CustomMLP(d_model, mlp_width, act_layer, tuning_mode=tuning_mode, init_ssf=init_ssf)
    ...
    def forward(
        ...
        if self.tuning_mode == 'ssf':
            x = q_x + self.ls_1(self.attention(q_x=ssf_ada(self.ln_1(q_x), self.ssf_scale_1, self.ssf_shift_1),
                                               k_x=k_x,
                                               v_x=v_x,
                                               attn_mask=attn_mask))
        else:
        ...
        if self.tuning_mode == 'ssf':
            x = x + self.ls_2(self.mlp(ssf_ada(self.ln_2(x), self.ssf_scale_2, self.ssf_shift_2)))
        else:
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.heads = heads
        tuning_mode_list = ['ssf' if i >= layers - tuning_layers else None for i in range(layers)]
        self.resblocks = nn.ModuleList([
            ResidualAttentionBlock(
                width, heads, mlp_ratio, ls_init_value=ls_init_value, act_layer=act_layer, norm_layer=norm_layer,
            tuning_mode=tuning_mode_list[i], init_ssf=init_ssf)
...
class TextTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = Transformer(
            ...
            # IMPORTANT: SSF based Parameters are below.
            # tuning_layers = 2 for Camelyon16/ TCGA-Lung datasets and
            # tuning_layers = 8 for UBC-OCEAN dataset
            tuning_layers=2,
            init_ssf=True,
        )
